            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="card-title">Image Information List</h4>
                        <a class="btn btn-primary" href="/imageInformation/create">
                            <i class="bi bi-plus-circle mx-1"></i>Add New Image Information
                        </a>
                    </div>
                    @if (count($imageInformation) > 0)
                    <div class="card-body">
                        @if (count($errors) > 0)
                        <div
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    @foreach ($imageInformation as $c)
                                    <tr>
                                        <td>
                                            {{ ($imageInformation->currentPage() - 1) * $imageInformation->perPage() +
                                            $loop->iteration }}
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-right">
                                            <a href="{{ route('imageInformation.show', $c->id) }}"><i
                                                    class="now-ui-icons ui-1_zoom-bold"></i></a>
                                            <a href="{{ route('imageInformation.edit', $c->id) }}"><i
                                                ...
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content" style="border-radius:1.3rem;border:none">
                                                        ...
                                                        <div class="modal-footer" style="border-top:none">
                                                            ...
                                                            <form action="{{ route('imageInformation.destroy', $c->id) }}"
                            ...
                            <nav aria-label="Page navigation example">
                                <ul class="pagination justify-content-end pt-4">
                                    @if ($imageInformation->currentPage() > 1)
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $imageInformation->previousPageUrl() }}" tabindex="-1"
                                            style="color:#c1510c">
                                    ...
                                    @for ($i = 1; $i <= $imageInformation->lastPage(); $i++)
                                        <li class="page-item {{ $i == $imageInformation->currentPage() ? 'active' : '' }}">
                                            <a class="page-link" href="{{ $imageInformation->url($i) }}"
                                                style="color:#c1510c;{{ $i == $imageInformation->currentPage() ? 'color:white;background-color:#c1510c;border:#c1510c' : '' }}">
                                                {{ $i }}
                                            </a>
                                        </li>
                                        @endfor

                                        @if ($imageInformation->currentPage() < $imageInformation->lastPage())
                                            <li class="page-item">
                                                <a class="page-link" href="{{ $imageInformation->nextPageUrl() }}"
                                                    style="color:#c1510c">
