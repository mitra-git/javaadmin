            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="card-title">News Image List</h4>
                        <a class="btn btn-primary" href="/newsImage/create">
                            <i class="bi bi-plus-circle mx-1"></i>Add New News Image
                        </a>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    @foreach ($newsImage as $c)
                                    <tr>
                                        <td>
                                            {{ ($newsImage->currentPage() - 1) * $newsImage->perPage() +
                                            $loop->iteration }}
                                        </td>
                                        <td>
                                            {{$c->news->title}}
                                        ...
                                        <td class="text-right">
                                            <a href="{{ route('newsImage.show', $c->id) }}"><i
                                                    class="now-ui-icons ui-1_zoom-bold"></i></a>
                                            <a href="{{ route('newsImage.edit', $c->id) }}"><i
                                                ...
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content" style="border-radius:1.3rem;border:none">
                                                        ...
                                                        <div class="modal-footer" style="border-top:none">
                                                            ...
                                                            <form action="{{ route('newsImage.destroy', $c->id) }}"
                            ...
                            <nav aria-label="Page navigation example">
                                <ul class="pagination justify-content-end pt-4">
                                    @if ($newsImage->currentPage() > 1)
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $newsImage->previousPageUrl() }}" tabindex="-1"
                                            style="color:#c1510c">
                                    ...
                                    @for ($i = 1; $i <= $newsImage->lastPage(); $i++)
                                        <li class="page-item {{ $i == $newsImage->currentPage() ? 'active' : '' }}">
                                            <a class="page-link" href="{{ $newsImage->url($i) }}"
                                                style="color:#c1510c;{{ $i == $newsImage->currentPage() ? 'color:white;background-color:#c1510c;border:#c1510c' : '' }}">
                                                {{ $i }}
                                            </a>
                                        </li>
                                        @endfor

                                        @if ($newsImage->currentPage() < $newsImage->lastPage())
                                            <li class="page-item">
                                                <a class="page-link" href="{{ $newsImage->nextPageUrl() }}"
                                                    style="color:#c1510c">
