            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a class="btn btn-primary" href="/projectImage/create">
                            <i class="bi bi-plus-circle mx-1"></i>Add New Project Image
                        </a>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    @foreach ($projectImage as $c)
                                    <tr>
                                        <td>
                                            {{ ($projectImage->currentPage() - 1) * $projectImage->perPage() +
                                            $loop->iteration }}
                                        </td>
                                        <td>
                                            {{$c->project->title}}
                                        ...
                                        <td class="text-right">
                                            <a href="{{ route('projectImage.show', $c->id) }}"><i
                                                    class="now-ui-icons ui-1_zoom-bold"></i></a>
                                            <a href="{{ route('projectImage.edit', $c->id) }}"><i
                                                ...
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content" style="border-radius:1.3rem;border:none">
                                                        ...
                                                        <div class="modal-footer" style="border-top:none">
                                                            ...
                                                            <form action="{{ route('projectImage.destroy', $c->id) }}"
                            ...
                            <nav aria-label="Page navigation example">
                                <ul class="pagination justify-content-end pt-4">
                                    @if ($projectImage->currentPage() > 1)
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $projectImage->previousPageUrl() }}" tabindex="-1"
                                            style="color:#c1510c">
                                    ...
                                    @for ($i = 1; $i <= $projectImage->lastPage(); $i++)
                                        <li class="page-item {{ $i == $projectImage->currentPage() ? 'active' : '' }}">
                                            <a class="page-link" href="{{ $projectImage->url($i) }}"
                                                style="color:#c1510c;{{ $i == $projectImage->currentPage() ? 'color:white;background-color:#c1510c;border:#c1510c' : '' }}">
                                                {{ $i }}
                                            </a>
                                        </li>
                                        @endfor

                                        @if ($projectImage->currentPage() < $projectImage->lastPage())
                                            <li class="page-item">
                                                <a class="page-link" href="{{ $projectImage->nextPageUrl() }}"
                                                    style="color:#c1510c">
